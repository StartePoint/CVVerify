...
namespace {
cv::Mat renderHistogramBarCanvas(
    ...
    bool strokeStyle)
{
    ...
    for (std::size_t index = 0; index < histograms.size(); ++index) {
        ...
        for (int i = 0; i < bins; ++i) {
            ...
            cv::rectangle(
                canvas,
                cv::Rect(x, y, width, std::max(1, baseY - y)),
                colors[index],
                strokeStyle ? 1 : cv::FILLED
            );
        }
    }
...
QString HistogramBarOperator::id() const
{
    return "builtin.histogram_bar";
}

QString HistogramBarOperator::displayName() const
{
    return "Histogram Bar";
}

StepSchema HistogramBarOperator::schema() const
{
    return {
        ...
        "Histogram Bar",
        {
            ...
            {"barStyle", "Bar Style", "图表设置", StepParameterType::Choice, "filled", {}, {}, {}, {}, {}, {
                {"filled", "Filled"},
                {"stroke", "Stroke"},
            }},
...
QVariantMap HistogramBarOperator::parameterValues() const
{
    return {
        {"channels", m_channels},
        {"bins", m_bins},
...
void HistogramBarOperator::setParameterValues(const QVariantMap& values)
{
    ...
    if (values.contains("barStyle")) m_barStyle = values.value("barStyle").toString();
    if (values.contains("chartMode")) m_chartMode = values.value("chartMode").toString();
}

StepResult HistogramBarOperator::execute(FramePacket& frame, const RunContext&)
{
    ...
    const cv::Mat chart = renderHistogramBarCanvas(
        histograms,
        colors,
        std::max(8, m_bins),
        m_normalize,
